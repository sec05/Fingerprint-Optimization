	...
	while ((entry = readdir(folder)))
	{
		...
		while (ptr != NULL)
		{
			...
			for (int i = 0; i < natoms; i++)
			{
				...
				while ((words2[nwords1++] = strtok(NULL, " ,\t")))
					continue;
				nwords1--;
				if (nwords1 != nwords - 2)
				{
...
#pragma omp for schedule(guided)
		for (nn = 0; nn < nsims; nn++)
		{
			...
			for (ii = 0; ii < sims[nn].inum; ii++)
			{
				...
				if (doscreen)
				{
					screen(Sik, Bij, ii, nn, xn, yn, zn, tn, jnum - 1); // jnum is neighlist + self term, hence jnum-1 in function inputs
				}
				if (allscreen)
				{
					screen_neighbor_list(xn, yn, zn, tn, &jnum, jl, i, nn, Bij, Sik);
				}
				...
				for (j = 0; j < len; j++)
				{
					if (fingerprints[itype][j]->spin == false && fingerprints[itype][j]->screen == false)
						fingerprints[itype][j]->compute_fingerprint(features, ii, nn, xn, yn, zn, tn, jnum - 1, jl);
					else if (fingerprints[itype][j]->spin == false && fingerprints[itype][j]->screen == true)
						fingerprints[itype][j]->compute_fingerprint(features, Sik, Bij, ii, nn, xn, yn, zn, tn, jnum - 1, jl);
				...
				for (j = 0; j < len; j++)
				{
					if (fingerprints[itype][j]->spin == false && fingerprints[itype][j]->screen == false)
						...
						fingerprints[itype][j]->compute_fingerprint(features, Bij, ii, nn, xn, yn, zn, tn, jnum - 1, jl);
					else if (fingerprints[itype][j]->spin == true && fingerprints[itype][j]->screen == false)
						fingerprints[itype][j]->compute_fingerprint(features, ii, nn, xn, yn, zn, tn, jnum - 1, jl);
					else if (fingerprints[itype][j]->spin == true && fingerprints[itype][j]->screen == true)
						fingerprints[itype][j]->compute_fingerprint(features,  Sik,  Bij, ii, nn, xn, yn, zn, tn, jnum - 1, jl);
...
void PairRANN::screen_neighbor_list(double *xn, double *yn, double *zn, int *tn, int *jnum, int *jl, int i, int sn, bool *Bij, double *Sik)
{
...
void PairRANN::screen(double *Sik, bool *Bij, int ii, int sid, double *xn, double *yn, double *zn, int *tn, int jnum)
{
	// #pragma omp parallel
	{
		...
		for (kk = 0; kk < jnum; kk++)
		{ // outer sum over k in accordance with source, some others reorder to outer sum over jj
...
std::vector<std::string> PairRANN::tokenmaker(std::string line, std::string delimiter)
{
	...
	while ((words[nwords++] = strtok(NULL, const_cast<char *>(delimiter.c_str()))))
		continue;
	nwords--;
